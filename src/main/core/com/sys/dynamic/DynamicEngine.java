...
public class DynamicEngine {
    ...
    public void javaCodetoClassFile(String fullClassName, String javaCode,File destFile){
        long start = System.currentTimeMillis(); //记录开始编译时间
        Object instance = null;
        //获取系统编译器
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        ...
        if (success) {
            ...
            JavaClassObject jco = fileManager.getJavaClassObject();
            try{
                FileCopyUtils.copy(jco.getBytes(),destFile);
            }catch (Exception e){
    ...
    public void javaCodetoClassFile(List<String> classNameList, List<JavaFileObject> jfiles,Map<String,File> destFile){
        long start = System.currentTimeMillis(); //记录开始编译时间
        Object instance = null;
        //获取系统编译器
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        // 建立DiagnosticCollector对象
        DiagnosticCollector<JavaFileObject> diagnostics = new DiagnosticCollector<JavaFileObject>();
       /* for(JavaFileObject jf:jfiles){
            diagnostics.report(jf);
        }*/
        ...
        options.add("-classpath");
        options.add(this.classpath);
        JavaCompiler.CompilationTask task = compiler.getTask(null, fileManager, diagnostics, options, classNameList, jfiles);
        // 编译源程序
        boolean success = task.call();
        ...
        if (success) {
            ...
            List<JavaClassObject> list=fileManager.getJavaClassObjects();
            try{
                for(JavaClassObject javaClassObject:list){
                    FileCopyUtils.copy(javaClassObject.getBytes(),destFile.get(javaClassObject.getClassName()));
                }
